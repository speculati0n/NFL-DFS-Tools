...
class PGAgent:
    ...
    def act(self, mask):
        ...
        logits = self.net(x)  # keep grad for update
        logits_np = logits.detach().numpy().flatten()

        # Determine valid actions; fall back to uniform random if mask is empty
        valid = np.where(mask == 1)[0]
        if len(valid) == 0:
            a = int(np.random.choice(self.n))
        else:
        ...
        self.last = (logits, a, torch.tensor(mask, dtype=torch.float32).unsqueeze(0))
        return a
